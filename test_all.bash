...
function recreateSale() {
    ...
    saleId=$(curl -X POST http://$HOST:$PORT/api/v1/customers/$customerId/purchases -H "Content-Type:
    application/json" --data "$aggregate" | jq '.saleId')
    allTestSaleId[$testId]=$saleId
    echo "Added Sale with saleId: ${allTestSaleId[$testId]}"
}
...
waitForService curl -X DELETE http://$HOST:$PORT/api/v1/customers/cus02
...
assertCurl 200 "curl http://$HOST:$PORT/api/v1/customers/cus01/purchases/${allTestSaleId[1]} -s"
